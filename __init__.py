...
__author__ = 'robconnolly, btotharye, nielstron, tptnotf'
...
class HomeAssistantSkill(FallbackSkill):
    ...
    def handle_light_increase_intent(self, message):
        self.log.debug("Increase light intensity on entity: " + message.data.get("entity"))
        message.data["Entity"] = message.data.get("entity")
    ...
    def handle_light_decrease_intent(self, message):
        self.log.debug("Decrease light intensity on entity: " + message.data.get("entity"))
        message.data["Entity"] = message.data.get("entity")
    ...
    def _handle_light_set(self, message):
        ...
        try:
            brightness_req = float(message.data.get("brightnessvalue"))
            if brightness_req > 100 or brightness_req < 0:
                self.speak_dialog('homeassistant.brightness.badreq')
        except KeyError:
            brightness_req = 10.0
        brightness_value = int(brightness_req / 100.0 * 255)
    ...
    def handle_shopping_list_intent(self, message):
        self.log.debug("Shopping list intent on entity: " + message.data.get("entity"))
        entity = message.data.get("entity")
        ha_data = {'name': entity}
    ...
    def handle_automation_intent(self, message):
        self.log.debug("Automation intent on entity: " + message.data["Entity"])
        entity = message.data["Entity"]
        ha_entity = self._find_entity(
            entity,
